...
// Pin a person's OWN board(s) by exact Asana gid — wins over (and is exclusive
// of) name matching, so look-alike boards can't be confused. Dhairya has SIX
// "Dhairya …" boards; this nails his to-do board to the right gid.
const OWN_BOARD_GIDS = { "dhairya agarwal": ["1210376806823569"] };
function boardBelongsTo(board, assignee) {
  ...
  // gid pin: a pinned board belongs ONLY to its pinned owner.
  if (gid) {
    for (const [person, gids] of Object.entries(OWN_BOARD_GIDS)) {
      if (gids.includes(String(gid))) return person === a;
    }
  }
