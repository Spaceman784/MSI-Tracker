...
function boardBelongsTo(boardName, assignee) {
...
export async function fetchRecurring(token, pinnedWorkspaceGid, { maxBoards = 0, log = () => {} } = {}) {
  ...
  await pool(boards, 8, async (b) => {
    ...
    for (const kind of Object.keys(KIND_RE)) {
      const sec = sections.find((s) => KIND_RE[kind].test(s.name) && !/^archived/i.test((s.name || "").trim()));
      if (!sec) continue;
      let secTasks = [];
      try {
      ...
      for (const t of secTasks) {
        ...
        if (assignee && !boardBelongsTo(b.name, assignee)) continue;
        tasks.push({
          gid: t.gid,
