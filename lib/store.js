...
export async function getDataset(force = false) {
  ...
  for (;;) {
    const { data, error } = await sb
      .from("mis_tasks")
      .select("gid,name,assignee,project,projects,section,sections,completed,completed_at,created_at,due_on")
...
/** A task's project list (handles old rows that only have a single `project`). */
function taskProjects(t) {
  return t.projects && t.projects.length ? t.projects : t.project ? [t.project] : [];
}
function taskSections(t) {
  return t.sections && t.sections.length ? t.sections : t.section ? [t.section] : [];
}
...
export function buildFilterLists(dataset) {
  ...
  tasks.forEach((t) => {
    taskProjects(t).forEach((p) => projects.add(p));
    taskSections(t).forEach((s) => sections.add(s));
  });
  return {
...
export function filterTasks(dataset, p) {
  ...
  return dataset.tasks.filter((t) => {
    ...
    if (p.project && p.project !== "All" && !taskProjects(t).includes(p.project)) return false;
    if (p.section && p.section !== "All" && !taskSections(t).includes(p.section)) return false;
...
export function aggregate(filtered) {
  ...
  for (const t of filtered) {
    ...
    r.total++;
    // Per-project breakdown: a multi-homed task is shown under each of its
    // projects (a project view), but KPIs/per-person below count it ONCE.
    for (const pr of taskProjects(t)) {
      if (!byP[pr]) byP[pr] = { name: pr, Total: 0, Completed: 0 };
      byP[pr].Total++;
      if (t.completed) byP[pr].Completed++;
    }
    if (t.completed) {
      completed++;
      r.completed++;
